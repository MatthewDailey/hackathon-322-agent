 ...
 */
async function executeWithErrorHandling<T>(action: () => Promise<T>, errorMsg: string): Promise<T> {
  try {
    return await action()
  } catch (error) {
...
export namespace Computer {
    ...
    switch (toolParams.action) {
      ...
      case 'hold_key': {
        if (!toolParams.text || !toolParams.duration) {
          throw new Error('Text and duration parameters are required for hold_key action')
        }
      ...
      case 'mouse_move': {
        if (!toolParams.coordinate) {
          throw new Error('Coordinate parameter is required for mouse_move action')
        }
        return moveMouse(toolParams.coordinate[0], toolParams.coordinate[1])
      }
      case 'left_mouse_down': {
        if (!toolParams.coordinate) {
          throw new Error('Coordinate parameter is required for left_mouse_down action')
        }
        return mouseDown(toolParams.coordinate[0], toolParams.coordinate[1])
      }
      case 'left_mouse_up': {
        if (!toolParams.coordinate) {
          throw new Error('Coordinate parameter is required for left_mouse_up action')
        }
        return mouseUp(toolParams.coordinate[0], toolParams.coordinate[1])
      }
      case 'left_click': {
        if (!toolParams.coordinate) {
          throw new Error('Coordinate parameter is required for left_click action')
        }
        if (toolParams.text) {
      ...
      case 'right_click': {
        if (!toolParams.coordinate) {
          throw new Error('Coordinate parameter is required for right_click action')
        }
      ...
      case 'double_click': {
        if (!toolParams.coordinate) {
          throw new Error('Coordinate parameter is required for double_click action')
        }
        return doubleClickAt(toolParams.coordinate[0], toolParams.coordinate[1])
      }
      case 'triple_click': {
        if (!toolParams.coordinate) {
          throw new Error('Coordinate parameter is required for triple_click action')
        }
        return tripleClickAt(toolParams.coordinate[0], toolParams.coordinate[1])
      }
      case 'scroll': {
        if (!toolParams.coordinate || !toolParams.scroll_amount || !toolParams.scroll_direction) {
          throw new Error(
   ...
   * @returns Promise that resolves with a success message
   */
  export async function holdKey(key: string, duration: number): Promise<string> {
    return executeWithErrorHandling(async () => {
